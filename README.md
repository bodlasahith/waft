...
- **Onboarding minimization** — cutting the "stranger → onboarded + connected" funnel toward the native-app floor (install once + one identity gesture + everything else auto):
  ...
  - [ ] **Sign in with Apple** — one-tap Face-ID auth (no typing, name auto-derived); also App Store Guideline 4.8-required once Google is offered. Unblocked now the Apple Developer account exists. Highest remaining auth win.
