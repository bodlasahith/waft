...
- [x] Live demo at YC AI Startup School
...
- [ ] **Age assurance for 18+/21+ events** (if needed for professional conferences / adult / alcohol-served events) — **gate at the event level, NOT globally.** A host marks an event `18+`/`21+`; only then must an attendee pass age assurance to check in. Do NOT add a DOB field to signup — collecting DOB from every user for a some-events need drags in COPPA / state age-verification / GDPR-minor obligations and adds onboarding friction we deliberately minimized. Tiered assurance, matched to legal stakes: (1) self-attested "over 18" checkbox (near-zero friction, weak — informal 18+ networking); (2) **platform age APIs** — Apple Declared Age Range API / Google equivalent return an age *range* with no DOB stored (privacy-preserving, low friction — the right default); (3) third-party ID scan / age estimation (Persona/Veriff/Yoti — heavyweight, paid, real privacy exposure; reserve for 21+ where law demands verified ID). **Minimization rule:** store only a `verified_18`/`verified_21` boolean (+ method), **never the DOB or ID image** — pass the ID to the vendor, keep only the yes/no. Same event-scoped, PII-minimizing philosophy as tiered-visibility and the both-attended guard.
...
- [ ] Push notifications ("X just joined Waft", "X just posted on Y platform you share in common", "X is organizing a Waft event")
...
- **Onboarding minimization** — cutting the "stranger → onboarded + connected" funnel toward the native-app floor (install once + one identity gesture + everything else auto):
  ...
  - [ ] **Pre-fill socials on signup** — goal: cut manual social entry by auto-populating a user's platforms at account creation. **NOT via scraping.** Two blockers, the first fatal regardless of effort: (1) *identity resolution* — from an email + name you can't reliably know which public accounts are theirs, so you'd risk attaching a **stranger's** handles = a privacy incident worse than showing nothing; there's no legitimate public API mapping email→handles. (2) *ToS/legal* — Instagram/TikTok/LinkedIn prohibit scraping; Meta blocks it, LinkedIn litigates; a real reputational/legal risk for a trust-and-consent product. The legitimate path is **consented OAuth "Connect your accounts"** (tap Connect Instagram/Google/GitHub/Spotify/LinkedIn → official API returns the handle *with permission*) — this **subsumes the "OAuth connect" item above** and is the real onboarding-speed win, but each provider needs its own app registration + review (Instagram Basic Display deprecated; LinkedIn/Meta require review) → **not a before-demo task.** Middle option, weak: paid B2B enrichment APIs (Clearbit/People Data Labs) keyed on email — thin consumer-social coverage, paid, and auto-attaching enrichment carries its own GDPR/CCPA consent exposure. **Explicitly rejected: ToS-violating scraping of third-party platforms.**
...
- [ ] Gamification badges — **earned**, not bought: derive from the real graph signal `computeGraphStats` already produces (top connector, cluster bridge, most-scanned, first-to-N-connections, cross-event regular). Display a chosen badge on the card/node. This is the utility-grounded analog of Partiful's "custom profile badge (achievements)" — ours reflect actual networking, not cosmetics
...
### Post-validation slice — profile identity + earned status (gated behind customer discovery)
Pulled forward from the items above after a Partiful premium-features survey (2026-07) confirmed willingness-to-pay for profile customization / self-expression — but on the **individual/retention** axis, not the host wedge. **Steal the utility-adjacent features, leave the vanity** (Partiful's list is ~80% cosmetic — pets, hats, animated themes, seasonal app icons, Pong; competing there means fighting a party-app on its own turf). These three overlap Waft's real value (network utility + validated demand) and are the ones to queue **once Thursday's discovery says which user — host or individual — to build for.** Do NOT start before that signal.
- [ ] **Earned achievement badges** — see "Gamification badges" above. The strongest overlap: our badges come from actual graph analytics, not a store. Individual-retention feature.
- [ ] **Node avatars v2** — accessories/accents on the abstract node (extends avatars v1). Fits Waft's shape-based identity where Partiful's photo/hat avatars wouldn't; keeps the wall legible.
- [ ] **Light/dark theme toggle (mobile)** — dark is already the default and the visual system is token-based; a user toggle is cheap and the survey validated demand. Custom colors/fonts are explicitly out (off-brand, unbounded).
- Explicitly rejected from the Partiful list: profile pets, custom app-icon drops, animated custom themes, RSVP "peek", generic mini-games (Pong/Snake) — vanity/off-domain. A *networking* mini-game (find-the-person icebreaker) is the only on-brand cousin, and it belongs with icebreakers, not here.
...
- [ ] **Edge `eventId` (scalar) → `eventIds` (set)** — a WAFT edge carries one `eventId` slot with last-write-wins (`SET r.eventId = coalesce($eventId, r.eventId)` in `graph.ts`). One relationship, one edge is correct; one *event* is not. If the same pair scans at Event A then Event B, the tag flips to B and the edge **silently vanishes from Event A's wall**; a wild→event transition also erases the "in the wild" origin. Fix (Neo4j-local, no store change): make it `r.eventIds` (append on each event scan), wall query `WHERE $eventId IN r.eventIds`, "in the wild" = empty list; optional `r.lastMetAt` / met-count for a "you keep crossing paths" signal (kept *out* of `strength`, which stays interaction-based). Harmless for single-event YC; **do before running two events with overlapping attendees.** Strictly smaller than the full store-consolidation below, and a subset of its benefit. Guard added 2026-07-22: `/connections` now requires *both* parties to have an ATTENDED edge before event-tagging (scanner-only → still connects, as a plain mutual).
...
### From product-vision consult (GPT, 2026-07 — see `docs/GPT_VISION.md`, `docs/GPT_ANALYSIS.md`)
External consult; independently reached "the graph is the product, the QR is the front door" (validates the [[waft-positioning]] individual-graph-as-moat reframe). Most of its advice restates our existing direction — parked here are only the genuinely *additive* ideas, with build/friction notes. **All gated behind customer discovery** (the consult is feature-building; validation comes first).
- [ ] **Event Replay / the time dimension** — the standout. We render the graph growing *live* (WS node-pops) but don't *record + replay* it. Scrub an event's network from 9am (one node) to 4pm (full room). **Nearly free given existing data** — every WAFT/ATTENDED edge already stores `createdAt`, so the timeline is reconstructable without new writes. Doubles as a killer host-outreach artifact ("watch your event's network emerge"). Front of the post-validation queue.
- [ ] **Graph lenses (relationship-dimension re-clustering)** — distinct from "Network visualization themes" above (those are aesthetic *renderers*); lenses *reorganize* the same graph by a chosen dimension — Nearby / Recently Met / This Conference / College — the layout re-clusters, Apple-Maps-mode style. New interaction, heavier build.
- [ ] **Composition insights (semantic, not structural)** — "you met 4 engineers, 3 founders, 2 designers this week" vs. our current structural stats (degree/betweenness/leaderboard). Caveat: needs a role/type on profiles — only build if **inferred** (from linked LinkedIn/GitHub) rather than *asked*, to avoid re-adding the onboarding friction we cut.
- [ ] **Ambient AI relationship-memory (backward-looking)** — distinct from our forward-looking icebreakers: quietly resurface "you met Alex at React Summit, you both know Emily, you talked about Rust." Directly serves the retention / "why open Waft between events" problem. Needs a per-connection notes/context capture. Keep it ambient (behind the graph), not a chatbot.
- Noted-but-not-parked (already covered): enterprise/organizer dashboards → Phase 3 "Event organizer partnerships" + Revenue "Enterprise"; graph-first/event-context/Neo4j fit → existing architecture; postpone chat/messaging/feed → already our stance.
