...
- [ ] Optional password for returning users — let a user *set* a password after account creation (Settings → "Set a password"), then sign in with email+password as a faster alternative to OAuth (can be slow) or OTP (requires opening the email app each time). Deliberately opt-in and post-signup — NOT a signup method (password signup contradicts the 20-second-signup pitch). The `signInWithPassword` path + UI already exist (built for App Store review, gated to the review email); this feature would add a set-password screen and open the path to any account that sets one. Supabase `updateUser({ password })` on an authenticated session
